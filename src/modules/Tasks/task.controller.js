...
export const addTask = async (req, res, next) => {
  ...
  if (taskType === "text" && textBody) {
    ...
    return res
      .status(200)
      .json({ message: "success", status: 200, createdTask });
  } else if (
    taskType === "list" &&
    listItems &&
...
export const updateTask = async (req, res, next) => {
  ...
  if (taskType === "text" && textBody) {
    ...
    return res.status(200).json({ message: "success", status: 200, newTask });
  } else if (
    taskType === "list" &&
    listItems &&
    Array.isArray(listItems) &&
...
export const getPrivateTasks = async (req, res) => {
  ...
  try {
    ...
    const privateTasks = await Task.find(filter)
      .skip(skip)
      .limit(limit)
      .populate("categoryId");
    ...
    return res
      .status(500)
      .json({ message: "Error fetching private tasks", error });
  }
};
...
export const deleteTask = async (req, res, next) => {
  ...
  if (!deleted) {
    return res.status(409).json({ message: "Task Delete failed" });
  }
